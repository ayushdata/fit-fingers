$(() => {
    ...
    function settttime() {
        seconds--;
        $('#calctime').html(seconds.toString() + " sec");
        if (seconds > 0 ) {
        ...
        if (seconds == 0){
            $(".words").hide();
            $(".words").show();
            $(".words").html(`Your scored ${score} points`);
            $(".words").css('color', 'green');
        }
    }
    function pickrandom(){
        $(".words").html("");
        var charofwords = list[Math.floor(Math.random() * 2643)].split("");
        let i = 0;
        charofwords.forEach((letter) => {
            var span = $("<span></span>").addClass("span" + i.toString()).text(letter);
            $(".words").append(span);
            i++;
        });
        count = 0;
    ...
    function press(){
        $(document).keypress((e)=>{
            ...
            if ($(".words").text()[count] === character){
                $(".span" + count.toString()).css("background-color", "#0099ff");
                count++;
            }
            if (count === $(".words").text().length){
                pickrandom();
                score++;
                $("#calcscore").html(score);
            }
        });
    }
    $("#retry").click(() => {
    ...
    var list = ['therapeutic', 'friendly', 'delight', 'sky', 'long', 'slow', 'fasten', 'attack', 'steady', 'ambiguous', 'teeny', 'temper', 'dead', 'hard-to-find', 'servant', 'low', 'clam', 'grease', 'screw', 'discreet', 'adventurous', 'hurt',
